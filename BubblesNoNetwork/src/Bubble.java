...
public class Bubble {
    ...
    public Bubble(int X_position, int Y_position) {
...
//        if (getX_position() < 0 || getX_position() > Toolkit.getDefaultToolkit().getScreenSize().getWidth() - getSize() - 5) {
        if (getX_position() < 0 || getX_position() > Toolkit.getDefaultToolkit().getScreenSize().getWidth() - size) {
            lastCollision = null;
            X_Speed *= -1;
        }
        //for frame decoration enabled
//        if (getY_position() < 0 || getY_position() > Toolkit.getDefaultToolkit().getScreenSize().getHeight() - getSize() - 32) {
        if (getY_position() < 0 || getY_position() > Toolkit.getDefaultToolkit().getScreenSize().getHeight() - size) {
            lastCollision = null;
            Y_Speed *= -1;
        }
    }
...
//            Y_Speed *= otherBubble.getY_Speed() / Math.abs(otherBubble.getY_Speed()) * -1;
    ...
    public boolean isInPosition(int x_position, int y_position) {
        //for frame decoration enabled
//        if (Math.pow(this.getX_position() + size / 2 + 4 - x_position, 2) + Math.pow(this.getY_position() + size / 2 + 25 - y_position, 2) <= Math.pow(size / 2, 2)) {
        if (Math.pow(this.getX_position() + size / 2 - x_position, 2) + Math.pow(this.getY_position() + size / 2 - y_position, 2) <= Math.pow(size / 2, 2)) {
            return true;
        }
        return false;
    }
