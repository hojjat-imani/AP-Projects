...
//            bubbles.add(new Bubble(X_position - Bubble.size / 2 - 4, Y_position - Bubble.size / 2 - 25));
            bubbles.add(new Bubble(X_position - Bubble.size / 2, Y_position - Bubble.size / 2));
        }
    }
...
//        }
    ...
    public void paint(Graphics g) {
        ...
        for (Bubble bubble : bubbles) {
            ...
            g.fillOval(bubble.getX_position(), bubble.getY_position(), Bubble.size, Bubble.size);
        }
    ...
    private boolean hasCollision(Bubble bubble1, Bubble bubble2) {
        if (Math.pow(bubble1.getX_position() - bubble2.getX_position(), 2) + Math.pow(bubble1.getY_position() - bubble2.getY_position(), 2) < Math.pow(Bubble.size, 2) && Math.pow(bubble1.getX_position() - bubble2.getX_position(), 2) + Math.pow(bubble1.getY_position() - bubble2.getY_position(), 2) > 0.75 * Math.pow(Bubble.size, 2) && bubble1.getLastCollision() != bubble2) {
            return true;
        }
        return false;
    }
